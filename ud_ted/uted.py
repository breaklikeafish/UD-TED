#!/usr/bin/python3
"""
An implementation of the unordered tree edit distance.
...
def uted_astar(x_nodes, x_adj, y_nodes, y_adj, delta = None, heuristic = 1, verbose = False, timeout=None):
    ...
    while not Q.empty():
        ...
        if f_lo_parent > highest_lower_bound:
            ...
            raise ValueError('Internal error: Lower bound shrank during computation, indicating that it is not a true lower bound!')
        ...
        for j in remaining:
            ...
            if verbose:
                print('replacement with %d: g = %g, h_child = %g, h_sib = %g, h_sib_parent = %g -> f_lo = %g' % (j, g, h_child, h_sib, h_sib_parent, f_lo))

    return best_distance, best_solution, len(edist_nodes)



def uted_constrained(x_nodes, x_adj, y_nodes, y_adj, delta = None, verbose = False):
    ...
    for i in range(m-1, -1, -1):
        for j in range(n-1, -1, -1):
            ...
            if m_i == 0:
                ...
                if m_i == 1:
                    rep_options = np.zeros(n_j)
                    i_0 = x_adj[i][0]
                    for l in range(n_j):
                        j_l = y_adj[j][l]
                        rep_options[l] = D_tree[i_0, j_l] + D_forest[m, j] - D_tree[m, j_l]
                    rep_cost = np.min(rep_options)
                elif n_j == 1:
                    rep_options = np.zeros(m_i)
                    j_0 = y_adj[j][0]
                    for k in range(m_i):
                        i_k = x_adj[i][k]
                        rep_options[k] = D_tree[i_k, j_0] + D_forest[i, n] - D_tree[i_k, n]
                    rep_cost = np.min(rep_options)
                else:
                    # otherwise, we use the Hungarian algorithm.
                    ...
                    for k in range(m_i):
                        for l in range(n_j):
                            ...
                            C[k, l] = D_tree[x_adj[i][k], y_adj[j][l]]
                    for k in range(m_i):
                        # matching c with n_j + c means deleting the
                        # c'th child of i
                        C[k, n_j + k] = D_tree[x_adj[i][k], n]
                    for l in range(n_j):
                        # matching m_i + c with c means inserting the
                        # c'th child of j
                        C[m_i + l, l] = D_tree[m, y_adj[j][l]]
                    C[m_i:, n_j:] = 0.
                ...
                if verbose:
                    print('(%d, %d) forest options: del %g, ins %g, rep %g' % (i, j, del_cost, ins_cost, rep_cost))
            ...
            # compute minimum across deletion, insertion, and replacement
            if verbose:
                print('(%d, %d) tree options: del %g, ins %g, rep %g' % (i, j, del_cost, ins_cost, rep_cost))
